...
const getRank = (startIndex, parentElement) => {
  ...
  $.ajax({
    type: "GET",
    url: "/mypage-list",
    data: { kind: parentElement, start: startIndex }
  }).done(function (datas) {
    ...
    if (datas.cnt > 0) {
      ...
      $.each(datas.lists, function (index, value) {
        ...
        let openEle;
        if (value.open_flg) {
          openEle = '<div class="chip pink lighten-3">公開中</div>';
        } else {
          openEle = '<div class="chip">下書き</div>';
        }

        //* note XSSを想定し、対策コードを実装している (escapeHTML)
        let ele = '<article>'
          + '<div class="rank_main">'
          + '<figure>'
          + '<a href="post-edit?post_id=' + value.id + '&movie_id=' + value.movie_id + '">'
          ...
          + '</figure>'
          + '<div class="rank_sub">' + openEle
          + '<span class="mute_txt">投稿日:' + value.created_at + '</span>'
          + '<a href="post-edit?post_id=' + value.id + '&movie_id=' + value.movie_id + '">'
